...
<small>補正は次の試行だけに適用。低位置の腕を直接ジョグしません。手前=robot X−、奥=X＋、物理右=Y−、下=Z−。</small>
<div class=axis><button onclick="n('x',-1)">手前 X−</button><button onclick="n('y',-1)">右 Y−</button><button onclick="n('z',-1)">下 Z−</button><button onclick="n('x',1)">奥 X＋</button><button onclick="n('y',1)">左 Y＋</button><button onclick="n('z',1)">上 Z＋</button></div>
