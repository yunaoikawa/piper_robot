...
<div class=cams><img src="/stream/head.mjpg"><img src="/stream/left.mjpg"><img src="/stream/right.mjpg"></div>
...
<script>const token=new URLSearchParams(location.search).get('token')||'';async function cmd(command,payload={}){try{let r=await fetch('/api/command?token='+encodeURIComponent(token),{method:'POST',headers:{'content-type':'application/json'},body:JSON.stringify({command,payload})});status.textContent=JSON.stringify(await r.json(),null,2)}catch(e){status.textContent='通信失敗: '+e}}function n(axis,direction){let raw=custom.value;let mm=raw===''?+step.value:Math.abs(+raw);cmd('adjust',{axis,direction,step_mm:mm})}let es=new EventSource('/events?token='+encodeURIComponent(token));es.onmessage=e=>{status.textContent=JSON.stringify(JSON.parse(e.data),null,2)};</script></main></body></html>"""
