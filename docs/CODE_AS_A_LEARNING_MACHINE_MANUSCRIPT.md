...
![Qualitative verified-capability curve above progressively richer agent tool graphs](assets/code_as_learning_machine/tool_graph_learning_curve.png)

**Figure 13. Empirical capability and tool-graph structure changed together.**
The horizontal axis contains five audited program states rather than uniform
time, and the vertical axis is deliberately qualitative rather than an
invented success rate. Generic replay had no independent endpoint. Relative
door replay plus aperture and a 5 mm probe established a contact prefix;
registered RGB-D then supported verified opening and closing. A different,
larger graph acquired verified cap removal and held transport without a cap
action demonstration. The final release branch added code but did not increase
the verified capability because placement evidence was insufficient. The
plateau therefore visualizes evaluator discipline as well as diminishing
returns: structural complexity alone is not credited as performance.
...
| 13 | `tool_graph_learning_curve.png` and `.svg` | Qualitative synthesis generated by `docs/generate_code_learning_tool_graph_figure.py` from the audited Door and Cap milestones in Sections 5--7; no numerical success-rate samples are implied |
