    ...
    def _transform_observation_to_lerobot_format(
        ...
        if "images" in obs:
            ...
            if expected_keys and len(expected_keys) == 1:
                ...
                # Model expects observation.image.{cam_name} format
                for cam_name, img in obs["images"].items():
                    img_tensor = process_image(img)
                    lerobot_cam_name = CAM_NAME_MAPPING.get(cam_name, cam_name)
                    lerobot_obs[f"observation.image.{lerobot_cam_name}"] = img_tensor
